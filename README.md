...
- **Higher-order / Isabelle / THF exporters** (`unicode_fol_kit.hol`) — Benzmüller-style shallow embeddings of *every* non-fuzzy logic into HOL: a real loadable **Isabelle** theory + **THF** for the full modal family (agent-indexed epistemic/doxastic, deontic, temporal), plus FOL/MSFOL, K3/LP, second-order, and intuitionistic (Gödel–McKinsey–Tarski → S4). The exporters emit sound problem files; they don't themselves run a prover (the quantified logics are undecidable)
- **Prove the modal embeddings with a local Isabelle** (opt-in) — with Isabelle installed, `isabelle_decide_modal(φ, frame=…)` (`unicode_fol_kit.hol.isabelle_runner`) drives it to decide modal validity (prove-battery → `nitpick` countermodel → UNKNOWN), validated differentially against the `satisfies_modal` Kripke oracle; `isabelle_available()` / `find_isabelle()` / `check_theory()` round it out. Linux-first (Windows via the bundled Cygwin); everything else works without Isabelle
...
- **Three-valued K3 / LP.** `to_thf_k3lp(φ, system="K3")` / `to_isabelle_k3lp` (also `…_entailment`) encode the truth-value type, the strong-Kleene connective functions, and the designated set (`{1}` for K3, `{½, 1}` for LP), so emitted theorem-hood matches K3 / LP validity. The Isabelle lemma carries a **real proof that discharges** — case-exhaustion over the three truth values for a valid formula, an `exI` witness for a refutation — built against a live Isabelle in the test suite. Cross-checked against `kleene_value`.
